describe('Login and Proceed with search input from homepage', () => {
  ...
  it('logs in and posts ad', () => {
    ...
    cy.get('[data-cy="post-title-input"]').type('Test advertisement 3');
...
cy.get('[data-cy="post-city-select"]').then($city => {
  ...
  if (isDisabled || !hasValue) {
    ...
    // Select Country
    cy.get('[data-cy="post-country-select"] select')
      .should('be.visible')
      .select(1) // Index or value
      .then(() => {
        cy.get('body').then($body => {
          const stateExists = $body.find('[data-cy="post-state-select"] select').length > 0;
          const cityExists = $body.find('[data-cy="post-city-select"] select').length > 0;
