describe('from homepage navigate to ad details page- Flow Test', () => {
  ...
  function selectAdTypeAndHandleCategories() {
    ...
    cy.get('#sort-by option').then($adTypeOptions => {
      ...
      const selectedAdTypeIndex = 1; // Choose the first ad type
...
cy.wait(2000)
      ...
      cy.wait(5000); // Give time for categories to load
      ...
      cy.document().then((doc) => {
        ...
        cy.get('a[data-cy="explore-category"]', { timeout: 10000 }).then($categoryLinks => {
          ...
          const selectedCategoryIndex = 1; // Choose the first category
          ...
          cy.document().then((doc) => {
            ...
            cy.get('[data-cy="ad-item"]', { timeout: 10000 }).then($adsInCategory => {
              if ($adsInCategory.length === 0) {
                ...
                cy.wrap($adsInCategory.eq(1)).scrollIntoView().click();
